...
A Kubernetes operator that manages Web Application Firewall (WAF) deployments using Coraza, integrated with Istio via WASM plugins. CRDs include **RuleSource** (stores SecLang rule text as `type: Rule` or data files as `type: Data`), **RuleSet** (ordered `spec.sources` listing RuleSource names), and **Engine** (attaches a RuleSet to a Gateway via Istio WasmPlugin).

Data flow: `RuleSources + RuleSet → RuleSetReconciler → RuleSetCache (HTTP server) → WASM plugin in Envoy → traffic filtering`
...
- **RuleSetReconciler** — watches RuleSet + referenced RuleSources (`get`/`list`/`watch` on `rulesources` in its namespace only; no cluster-wide ConfigMap/Secret access for rule material). Aggregates rules and data files from those sources, validates via Coraza, checks for WASM-unsupported rules, stores in RuleSetCache.
...
- `api/v1alpha1/` — CRD types (Engine, RuleSet, RuleSource, DriverConfig)
...
- RuleSources (`type: Rule`): `coraza.io/validation: "false"` — skips per-source Coraza rule validation
