...
existing="$(oc get configmap "${configmap}" -n "${namespace}" \
  --ignore-not-found -o name)"

if [[ -z "${existing}" ]]; then
  oc create configmap "${configmap}" -n "${namespace}" \
    --from-literal=config.yaml=$'enableUserWorkload: true\n'
  echo "Created ${configmap} and enabled OpenShift User Workload Monitoring."
  exit 0
fi
