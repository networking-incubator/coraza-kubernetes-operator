...
| Step | File | Purpose |
|------|------|---------|
| 0 | `01-enable-user-workload-monitoring.sh` | Idempotent cluster-admin setup for User Workload Monitoring. Creates `cluster-monitoring-config` if needed. |
...
- OpenShift User Workload Monitoring enabled by a cluster administrator. The optional helper needs `cluster-admin`; it creates `cluster-monitoring-config` when needed and otherwise preserves its settings.
...
Engine for this test without changing `41-waf-engine.yaml`.
...
# Creates Grafana, an egress-restricted NetworkPolicy, and grants its
